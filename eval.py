...
parser.add_argument("--name", type=str, default="v2_20h", help="experiment name")
...
save_config["obstacle_config_path"] = os.path.join(save_env_config_dir,
                                                   os.path.basename(config["obstacle_config_path"]))
    ...
    while True:
        ...
        if done or step_cnt >= 500:
            ...
            if info["done_info"] == "finished":
                reach_counter += 1
                print(
                    Back.GREEN + f"{i_episode}/{EPISODE} | info: {info}| reward: {ep_r}| average_reward:{ep_r / step_cnt}")
            else:

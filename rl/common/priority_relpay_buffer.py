...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        min_prob = np.min(self.data.tree[-self.data.capacity:]) / self.data.total_p  # for later calculate ISweight
        if min_prob == 0:
            min_prob = 0.00001
        ...
        mini_batch = MiniBatch(s_batch,a_batch,r_batch,s_prime_batch,d_batch)
        return b_idx, mini_batch, ISWeights



    def store(self, state: np.ndarray, action, reward, next_state, done):
        max_p = np.max(self.data.tree[-self.data.capacity:])
        if max_p == 0:
            max_p = self.abs_err_upper
        transition = Transition(state,action,reward,next_state,done)
        self.data.add(max_p, transition)

    def batch_update(self, tree_idx, td_errors):
        td_errors += self.epsilon  # convert to abs and avoid 0
        clipped_errors = np.minimum(td_errors, self.abs_err_upper)
        ps = np.power(clipped_errors, self.alpha)
        for ti, p in zip(tree_idx, ps):
            self.data.update(ti, p)
